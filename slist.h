...
class slist:public llist {
  ...
  slist();   // constructor. cout "in slist constructor".
    ...
    el_t search2(el_t);
    // search through the list, return found element (el_t), if not found returns blank el_t.

    // search through the list, return found element (el_t) and removes it,
    // if not found returns blank el_t.
    el_t del(el_t);
